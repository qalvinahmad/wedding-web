...
export const AnimatedTestimonials = ({
  ...
  // Generate consistent rotation values for each testimonial
  const rotationValues = useMemo(() => {
    return testimonials.map((_, index) => {
      // Use index-based deterministic rotation instead of random
      const baseRotation = (index * 3) % 21 - 10; // Creates values between -10 and 10
      return baseRotation;
    });
  }, [testimonials]);
    ...
    return (
      <div className="mx-auto max-w-sm px-4 py-20 font-sans antialiased md:max-w-4xl md:px-8 lg:px-12">
        <div className="relative grid grid-cols-1 gap-20 md:grid-cols-2">
          <div>
            <div className="relative h-80 w-full bg-gray-200 rounded-3xl animate-pulse" />
          </div>
